...
function Navbar() {
  ...
  return <header className="nav-wrap"><nav className="nav shell" aria-label="Main navigation">
    <a href="#top" className="brand" aria-label="Home"><BrandMark/><strong>{portfolio.person.navName}</strong></a>
...
function Projects() { return <section id="projects" className="section shell"><SectionHeading number="03" eyebrow="What I’ve built" title="Projects"/><div className="projects-grid">{portfolio.projects.map(p => <article className="project-card reveal" key={p.title}><div className="project-cover">{p.image ? <img src={p.image} alt={`${p.title} project screenshot`} /> : <span>{p.title.split(" ").map(x => x[0]).join("").slice(0,3)}</span>}</div><div className="project-body"><p className="eyebrow">{p.category}</p><div className="tags">{p.technologies.map(t => <span key={t}>{t}</span>)}</div><h3>{p.title}</h3><p>{p.description}</p><div className="project-links">{p.demo && <a href={p.demo} target="_blank" rel="noreferrer"><ExternalLink/>{p.demoLabel || "Live demo"}</a>}{p.github && <a href={p.github} target="_blank" rel="noreferrer"><GitBranch/>GitHub</a>}{portfolio.projectsNote && <span>{portfolio.projectsNote}</span>}</div></div></article>)}</div></section>; }
...
export function PortfolioPage() {
  ...
  return <><Navbar/><main><Hero/>{s.about && <About/>}{s.experience && <Experience/>}{s.skills && <Skills/>}{s.projects && <Projects/>}{s.education && <Education/>}{s.contact && <Contact/>}</main><footer><div className="shell"><p>Built with Next.js, Thanks for visiting!</p><a href="#top">Back to top ↑</a></div></footer></>;
}
